...
export function getPartVariantImageUrl(type: string, name: string, colorSlug: string, productId?: string, subIdx?: number): string | null {
  ...
  if (productId && subIdx !== undefined) {
    ...
    if (dir) {
      ...
      // Check for sub-index: productId like "BX-35-1" has 2+ dashes, "CX-16" has 1
      const dashCount = (productId.match(/-/g) || []).length;
      const key = dashCount >= 2 ? productId : `${productId}-${subIdx}`;
